...
{
    /// <summary>
    /// Internal class to read LiteDB v4 database version (datafile v7 structure)
    /// </summary>
    internal class FileReaderV7 : IFileReader
    {
        ...
        public FileReaderV7(Stream stream, string password)
        {
            ...
            // only userVersion was available in old file format versions
        ...
        public IEnumerable<IndexInfo> GetIndexes(string collection)
        {
            ...
            {
                var indexDict = index.RawValue as Dictionary<string, BsonValue>;

                yield return new IndexInfo(new CollectionIndex
                {
                    Field = indexDict.GetOrDefault("name").AsString,
                    Expression = indexDict.GetOrDefault("expression").AsString,
                    Unique = indexDict.GetOrDefault("unique").AsBoolean
                });
            }
        }
        ...
        public IEnumerable<BsonDocument> GetDocuments(string collection)
        {
            ...
            var headPageID = (uint) col["indexes"].AsArray[0].AsDocument["headPageID"].AsInt32;
            ...
            foreach (var indexPageID in indexPages)
            {
                ...
                {
                    var dataBlock = (node.RawValue as Dictionary<string, BsonValue>).GetOrDefault("dataBlock");

                    // if datablock link to a data page
                    if (((Dictionary<string, BsonValue>) dataBlock)["pageID"].AsInt32 != -1)
                    {
                        // read dataPage and data block
                        var dataPage = this.ReadPage((uint) ((Dictionary<string, BsonValue>) dataBlock).GetOrDefault("pageID").AsInt32);

                        if (dataPage.GetOrDefault("pageType").AsInt32 != 4) continue;

                        var block = dataPage["blocks"].AsArray.FirstOrDefault(x => x.AsDocument["index"] == dataBlock.AsDocument["index"]).AsDocument;

                        if (block == null) continue;

                        // read byte[] from block or from extend pages
                        var data = block.AsDocument["extendPageID"].AsInt32 == -1 ? block.AsDocument["data"].AsBinary : this.ReadExtendData((uint) block.AsDocument["extendPageID"].AsInt32);
        ...
        private BsonDocument ReadPage(uint pageID)
        {
            ...
            if (page["pageType"] == 1)
            {
                ...
                if (string.CompareOrdinal(info, HeaderPage.HEADER_INFO) != 0 || ver != HeaderPage.FILE_VERSION)
                {
                ...
                for (var i = 0; i < cols; i++)
                {
                    ...
                    ((Dictionary<string, BsonValue>)page["collections"].RawValue)[name] = (int) colPageID;
                }
            }
        ...
        private HashSet<uint> VisitIndexPages(uint startPageID)
        {
            ...
            while (toVisit.Count > 0)
            {
                ...
                foreach (var node in indexPage["nodes"].AsArray)
                {
                    var prev = (uint) node.AsDocument["prev"].AsDocument["pageID"].AsInt32;
                    var next = (uint) node.AsDocument["next"].AsDocument["pageID"].AsInt32;
